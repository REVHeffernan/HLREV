...
const char *CBreakable::pSpawnObjects[] =
{
	...
	"weapon_herb_green",// 18
	"weapon_spray",		// 19
	"weapon_flamethrower",// 20
	"ammo_flameclip",	// 21
};
...
int CBreakable :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( pevAttacker == pevInflictor )
	{
		...
		// if a client hit the breakable with a combat knife, and breakable is combat knife-sensitive, break it now.
		if ( FBitSet ( pevAttacker->flags, FL_CLIENT ) &&
				 FBitSet ( pev->spawnflags, SF_BREAK_COMBAT_KNIFE ) && (bitsDamageType & DMG_CLUB))
			flDamage = pev->health;
	}
	else
	...
	// Breakables take double damage from the combat knife
...
void CBreakable::Die( void )
{
	...
	fvol = RANDOM_FLOAT(0.85, 1.0) + (v_abs(pev->health) / 100.0);
