...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] =
{
	...
	DEFINE_FIELD( CBasePlayer, m_flNextEntityInfoTime, FIELD_TIME ),

	DEFINE_FIELD( CBasePlayer, m_flStunTime, FIELD_TIME )
};
...
void LinkUserMessages( void )
{
	...
	gmsgEntInfo = REG_USER_MSG( "EntInfo", -1 );
	gmsgPaperTitle = REG_USER_MSG( "PaperTitle", -1 );
	gmsgPaper = REG_USER_MSG( "Paper", -1 );
...
//LRC
void CBasePlayer::RemoveItems( int iWeaponMask, int i9mm, int i357, int iBuck, int iBolt, int iARGren, int iRock, int iGren, int iMP5, int iHerbRed, int iHerbGreen, int iSpray, int iFlame )
{
	int i;
	...
	RemoveAmmo("MP5", iMP5);
	RemoveAmmo("HerbRed", iHerbRed);
	RemoveAmmo("HerbGreen", iHerbGreen);
	RemoveAmmo("Spray", iSpray);
	RemoveAmmo("Flame", iFlame);
	...
	for (i = 0; i < MAX_ITEM_TYPES; i++)
	{
		...
		while (pCurrentItem->m_pNext)
		{
			...
			{
				((CBasePlayerWeapon*)pCurrentItem)->DrainClip(this, FALSE, i9mm, i357, iBuck, iBolt, iARGren, iRock, iGren, iMP5, iHerbRed, iHerbGreen, iSpray, iFlame);
...
//				ALERT(at_console, "Removing %s. (id = %d)\n", pCurrentItem->m_pNext->pszName(), pCurrentItem->m_pNext->m_iId);
			...
			else
			{
				...
				((CBasePlayerWeapon*)pCurrentItem)->DrainClip(this, TRUE, i9mm, i357, iBuck, iBolt, iARGren, iRock, iGren, iMP5, iHerbRed, iHerbGreen, iSpray, iFlame);
...
void CBasePlayer::TabulateAmmo()
{
	...
	ammo_mp5 = AmmoInventory( GetAmmoIndex( "MP5" ) );
	ammo_herb_red = AmmoInventory( GetAmmoIndex( "HerbRed" ) );
	ammo_herb_green = AmmoInventory( GetAmmoIndex( "HerbGreen" ) );
	ammo_spray = AmmoInventory( GetAmmoIndex( "Spray" ) );
	ammo_flame = AmmoInventory( GetAmmoIndex( "Flame" ) );
}
...
void CBasePlayer::CheckTimeBasedDamage()
{
	...
	if (v_abs(gpGlobals->time - m_tbdPrev) < 2.0)
		return;
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	*/	case 97: //AJH - send USE_SPAWN
	...
	case 101:
		...
		GiveNamedItem( "weapon_herb_red" );
		GiveNamedItem( "weapon_herb_green" );
		GiveNamedItem( "weapon_spray" );
		GiveNamedItem( "weapon_flamethrower" );
		GiveNamedItem( "weapon_sti" );
		...
		GiveAmmo( M203_MAX_CARRY, "ARgrenades",	M203_MAX_CARRY );
		...
		GiveAmmo( MP5_MAX_CARRY, "MP5", MP5_MAX_CARRY );
		GiveAmmo( HEALTH_MAX_CARRY, "HerbRed", HEALTH_MAX_CARRY );
		GiveAmmo( HEALTH_MAX_CARRY, "HerbGreen", HEALTH_MAX_CARRY );
		GiveAmmo( HEALTH_MAX_CARRY, "Spray", HEALTH_MAX_CARRY );
		GiveAmmo( FLAME_MAX_CARRY, "Flame",	FLAME_MAX_CARRY );
...
int CBasePlayer::RemovePlayerItem( CBasePlayerItem *pItem )
{
	if (m_pActiveItem == pItem)
	{
		ResetAutoaim( );
		pItem->Holster( );
		pItem->DontThink(); // Combat knife may be trying to swing again, etc.
		pItem->SetThink( NULL );
		m_pActiveItem = NULL;
		pev->viewmodel = 0;
		pev->weaponmodel = 0;
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgTimer, NULL, ENT( pev ) );
			WRITE_LONG( 0 );
		MESSAGE_END();
...
TYPEDESCRIPTION	CStripWeapons::m_SaveData[] =
{
	...
	DEFINE_FIELD( CStripWeapons, m_iMP5, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iHerbRed, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iHerbGreen, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iSpray, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iFlame, FIELD_INTEGER )
};
...
void CStripWeapons :: KeyValue( KeyValueData *pkvd )
{
	...
	else if ( FStrEq( pkvd->szKeyName, "mp5" ) )
	{
		m_iMP5 = atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq( pkvd->szKeyName, "herbred" ) )
	{
		m_iHerbRed = atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq( pkvd->szKeyName, "herbgreen" ) )
	{
		m_iHerbGreen = atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq( pkvd->szKeyName, "spray" ) )
	{
		m_iSpray = atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq( pkvd->szKeyName, "flame" ) ) 
	{ 
	    m_iFlame = atoi( pkvd->szValue ); 
	    pkvd->fHandled = TRUE; 
	} 
	else
...
void CStripWeapons :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer )
	{
		pPlayer->RemoveItems( pev->spawnflags, m_i9mm, m_i357, m_iBuck, m_iBolt,
				m_iARGren, m_iRock, m_iGren, m_iMP5, m_iHerbRed, m_iHerbGreen, m_iSpray, m_iFlame );
...
void CBasePlayer::StunThink( void )
{
	float flSpeed;
	if ( m_flStunTime > gpGlobals->time )
		flSpeed = 0.0;
	else
	{
		m_flStunTime = -1.0;
		flSpeed = pev->health > 30 ? 256.0 : 96.0f;
	}
	CVAR_SET_FLOAT( "sv_maxspeed", flSpeed );
	g_engfuncs.pfnSetClientMaxspeed( edict(), flSpeed );
}

#include "CScientist.h"

void CBasePlayer::EntityInfoThink( void )
{
	if ( m_flNextEntityInfoTime < 0.0 || m_flNextEntityInfoTime > gpGlobals->time )
		return;

	TraceResult pTraceResult;
	UTIL_TraceLine( GetGunPosition(), GetGunPosition() + gpGlobals->v_forward * 4096.0, dont_ignore_monsters, dont_ignore_glass, ENT( pev ), &pTraceResult );

	CBaseEntity *pHit = CBaseEntity::Instance( pTraceResult.pHit );
	if ( pHit )
	{
		if ( strstr( STRING( pHit->pev->classname ), "monster_" ) )
		{
			CBaseMonster *pMonster = (CBaseMonster *)pHit;
			int iRelationship = -1;
			if ( FClassnameIs( pMonster->pev, "monster_scientist" ) )
			{
				CScientist *pSci = (CScientist *)pMonster;
				iRelationship = pSci->m_iCustomType == 0 ? 1 : pSci->m_iCustomType;
			}
			else
			{
				switch ( pMonster->Classify() )
				{
				case CLASS_HUMAN_PASSIVE: // Ally
				case CLASS_PLAYER_ALLY:
				case CLASS_PLAYER_BIOWEAPON:
					iRelationship = 1;
					break;
				case CLASS_MACHINE: // Enemy
				...
				case CLASS_BARNACLE:
					iRelationship = 3;
					break;  
				default: iRelationship = 2; break; // Neutral
				}
			}
			...
			{
				strcpy( cName, STRING( pMonster->pev->targetname ) );
				cName[0] = toupper( cName[0] );
			}
			else
				sprintf( cName, "#%s", STRING( pMonster->pev->classname ) );

			MESSAGE_BEGIN( MSG_ONE, gmsgEntInfo, NULL, ENT( pev ) );
				WRITE_STRING( cName );
				WRITE_LONG( pMonster->pev->deadflag == DEAD_NO ? (int)pMonster->pev->health : 0 );
				WRITE_BYTE( iRelationship );
			MESSAGE_END();
		}
		else
		{
			CBaseEntity *pEntity = UTIL_FindEntityInSphere( NULL, pTraceResult.vecEndPos, 8.0 );
			if ( pEntity && (strstr( STRING( pEntity->pev->classname ), "item_" ) != NULL || strstr( STRING( pEntity->pev->classname ), "weapon_" ) != NULL) )
			{
				char cName[256];
				if ( !FStringNull( pEntity->pev->targetname ) )
				{
					strcpy( cName, STRING( pEntity->pev->targetname ) );
					cName[0] = toupper( cName[0] );
				}
				else
					sprintf( cName, "#%s", STRING( pEntity->pev->classname ) );
	...
	m_flNextEntityInfoTime = gpGlobals->time + 1.0;
}
