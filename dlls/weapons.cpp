...
void DecalGunshot( TraceResult *pTrace, int iBulletType )
{
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		case BULLET_MONSTER_12MM:
			...
			UTIL_GunshotDecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET ) );
			break;
		case BULLET_PLAYER_COMBAT_KNIFE:
			// wall decal
...
void W_Precache(void)
{
	...
	// Red herb
	UTIL_PrecacheOtherWeapon( "weapon_herb_red" );

	// Green herb
	UTIL_PrecacheOtherWeapon( "weapon_herb_green" );
	...
	UTIL_PrecacheOtherWeapon( "ammo_flameammo" );
	...
	UTIL_PrecacheOtherWeapon( "weapon_sti" ); 
}
...
void CBasePlayerItem::DefaultTouch( CBaseEntity *pOther )
{
	...
	if ( !(pPlayer->m_afButtonPressed & IN_USE) && !gEvilImpulse101 ) //If we just impulse 101'd give us the items like normal
	{
		ClientPrint( pOther->pev, HUD_PRINTCENTER, "#PressUseToPickUp" );
		return;
	}
...
//=========================================================
void CBasePlayerWeapon::DrainClip(CBasePlayer* pPlayer, BOOL keep, int i9mm, int i357, int iBuck, int iBolt, int iARGren, int iRock, int iGren, int iMP5, int iHerbRed, int iHerbGreen, int iSpray, int iFlame )
{
	...
	else if (iPAI == pPlayer->GetAmmoIndex("MP5"))			iAmt = iMP5;
	else if (iPAI == pPlayer->GetAmmoIndex("HerbRed"))		iAmt = iHerbRed;
	else if (iPAI == pPlayer->GetAmmoIndex("HerbGreen"))	iAmt = iHerbGreen;
	else if (iPAI == pPlayer->GetAmmoIndex("Spray"))		iAmt = iSpray;
	else if (iPAI == pPlayer->GetAmmoIndex("Flame"))		iAmt = iFlame;
	else return;
...
//=========================================================
void CWeaponBox::WBTouch( CBaseEntity *pOther )
{
	if ( !(pev->flags & FL_ONGROUND ) )
	{
		return;
...
CBaseEntity *CWeaponBox::Respawn()
{
	SetTouch( NULL );
	pev->effects |= EF_NODRAW;

	UTIL_SetOrigin( this, pev->origin );// blip to whereever you should respawn.

	SetThink( &CWeaponBox::WBMaterialize );
	pev->nextthink = gpGlobals->time + 10.0f;
	return this;
}

void CWeaponBox::WBMaterialize()
{
	if ( pev->effects & EF_NODRAW )
	{
		// changing from invisible state to visible.
...
BOOL CWeaponBox::PackWeapon( CBasePlayerItem *pWeapon )
{
	...
	pWeapon->SetThink( NULL ); // Combat knife may be trying to swing again, etc.
