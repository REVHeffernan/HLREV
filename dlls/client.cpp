...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq( pcmd, "reset_camera" ) )
	{
		CBasePlayer *pPlayer = GetClassPtr( (CBasePlayer *)pev );
		if ( pPlayer )
		{
...
void UpdateClientData ( const struct edict_s *ent, int sendweapons, struct clientdata_s *cd )
{
	...
	if ( sendweapons )
	{
		...
		if ( pl )
		{
			...
			cd->ammo_cells		= pl->ammo_flame;
			cd->vuser2.x		= pl->ammo_mp5;
			cd->vuser3.x		= pl->ammo_herb_red;
			cd->vuser3.y		= pl->ammo_herb_green;
			cd->vuser3.z		= pl->ammo_spray;
