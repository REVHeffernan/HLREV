...
//	ALERT(at_console,"Making Monster NOW\n");
    ...
    CBaseEntity *pSpawnedMonster = Instance( pevCreate );
if ( pSpawnedMonster && pSpawnedMonster->pev->flags & FL_MONSTER )
{
    CBaseMonster *pSpawnedMonsterAsMonster = (CBaseMonster *)pSpawnedMonster;
    if ( pSpawnedMonsterAsMonster )
    {
        pSpawnedMonsterAsMonster->SetBodygroup( 0, RANDOM_LONG( 0, 23 ) ); // Body
          pSpawnedMonsterAsMonster->SetBodygroup( 1, RANDOM_LONG( 0, 17 ) ); // Legs
        pSpawnedMonsterAsMonster->SetBodygroup( 2, RANDOM_LONG( 0, 4 ) ); // Head
