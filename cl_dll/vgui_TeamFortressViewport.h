...
#define MAX_PAPER_TITLE_LENGTH	32
...
private:
	...
	int		m_iGotAllPaper;
	char	m_szPaper[MAX_PAPER_LENGTH];
...
public:
	...
	int MsgFunc_PaperTitle( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_Paper( const char *pszName, int iSize, void *pbuf );
...
public:
	...
	CVGUIEntityInfo *m_pEntityInfo;
	CVGUITimer		*m_pTimer;

	char			m_szServerName[ MAX_SERVERNAME_LENGTH ];
	char			m_szPaperTitle[MAX_PAPER_TITLE_LENGTH];
};
...
public:
	...
	virtual void actionPerformed(Panel* panel)
	{
		...
		gEngfuncs.pfnClientCmd( "reset_camera\n" );
	}
};
