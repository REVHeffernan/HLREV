...
// returns volume of strike instrument (combat knife) to play
float EV_HLDM_PlayTextureSound( int idx, pmtrace_t *ptr, float *vecSrc, float *vecEnd, int iBulletType )
{
	...
	case CHAR_TEX_COMPUTER:
		...
		break;
	case CHAR_TEX_FLESH:
		if ( iBulletType == BULLET_PLAYER_COMBAT_KNIFE )
			return 0.0; // Combat knife already makes this sound
...
void EV_FireBeretta( event_args_t *args )
{
	...
	vec3_t vecUp, vecRight, vecForward;
	
	iIndex = args->entindex;
	VectorCopy( args->origin, vecOrigin );
	VectorCopy( args->angles, vecAngkes );
	VectorCopy( args->velocity, vecVelocity );

	bIsEmpty = args->bparam1;
	AngleVectors( vecAngkes, vecForward, vecRight, vecUp );

	iShellMDL = gEngfuncs.pEventAPI->EV_FindModelIndex( "models/shell.mdl" );

	if ( EV_IsLocal( iIndex ) )
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( bIsEmpty ? BERETTA_SHOOT_EMPTY : BERETTA_SHOOT, 0 );

		V_PunchAxis( 0, -2.0 );
	}

	EV_GetDefaultShellInfo( args, vecOrigin, vecVelocity, vecShellVelocity, vecShellOrigin, vecForward, vecRight, vecUp, 20.0, -12.0, 4.0 );
	EV_EjectBrass ( vecShellOrigin, vecShellVelocity, vecAngkes[YAW], iShellMDL, TE_BOUNCE_SHELL ); 

	gEngfuncs.pEventAPI->EV_PlaySound( iIndex, vecOrigin, CHAN_WEAPON, "weapons/pl_gun3.wav", gEngfuncs.pfnRandomFloat( 0.92, 1.0 ), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );

	EV_GetGunPosition( args, vecSrc, vecOrigin );
	VectorCopy( vecForward, vecAiming );

	EV_HLDM_FireBullets( iIndex, vecForward, vecRight, vecUp, 1, vecSrc, vecAiming, 8192.0, BULLET_PLAYER_9MM, 0, 0, args->fparam1, args->fparam2 );
}
//======================
//	   BERETTA END
//======================
...
void EV_FireColtSAA( event_args_t *args )
{
	int iIndex;
	vec3_t vecOrigin;
	vec3_t vecAngles;
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( bIsEmpty ? COLTSAA_FIRE_LAST : COLTSAA_FIRE, 0 );

		V_PunchAxis( 0, -10.0 );
	}

	EV_GetDefaultShellInfo( args, vecOrigin, vecVelocity, vecShellVelocity, vecShellOrigin, vecForward, vecRight, vecUp, 20.0, -12.0, 4.0 );
	EV_EjectBrass ( vecShellOrigin, vecShellVelocity, vecAngles[YAW], iShellMDL, TE_BOUNCE_SHELL ); 

	switch ( gEngfuncs.pfnRandomLong( 0, 1 ) )
	{
	case 0: gEngfuncs.pEventAPI->EV_PlaySound( iIndex, vecOrigin, CHAN_WEAPON, "weapons/357_shot1.wav", gEngfuncs.pfnRandomFloat( 0.8, 0.9 ), ATTN_NORM, 0, PITCH_NORM ); break;
	case 1: gEngfuncs.pEventAPI->EV_PlaySound( iIndex, vecOrigin, CHAN_WEAPON, "weapons/357_shot2.wav", gEngfuncs.pfnRandomFloat( 0.8, 0.9 ), ATTN_NORM, 0, PITCH_NORM ); break;
	}

	EV_GetGunPosition( args, vecSrc, vecOrigin );
	VectorCopy( vecForward, vecAiming );

	EV_HLDM_FireBullets( iIndex, vecForward, vecRight, vecUp, 1, vecSrc, vecAiming, 8192.0, BULLET_PLAYER_357, 0, 0, args->fparam1, args->fparam2 );
}
//======================
//     COLT SAA END
//======================
...
void EV_FireMP5( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(  bIsEmpty ? TEC9_FIRE_SLIDEBACK : (TEC9_FIRE1 + gEngfuncs.pfnRandomLong(0,2)), 2 );
...
void EV_FireGrenadeLauncher( struct event_args_s *args )
{
	...
	if ( EV_IsLocal( iIndex ) )
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation( GLAUNCHER_FIRE, 0 );
		V_PunchAxis( 0, -10 );
	}
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( iIndex, vecOrigin, CHAN_WEAPON, "weapons/glauncher.wav", 1, ATTN_NORM, 0, 95 + gEngfuncs.pfnRandomLong( 0, 10 ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( iIndex, vecOrigin, CHAN_WEAPON, "weapons/glauncher2.wav", 1, ATTN_NORM, 0, 95 + gEngfuncs.pfnRandomLong( 0, 10 ) );
		break;
	}
}
...
void EV_FireFlamethrower( struct event_args_s *args )
{
	...
	if ( EV_IsLocal( iIndex ) )
	{
		...
		{
		case 0: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAMETHROWER_ALTFIRE1, 0 ); break;
		case 1: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAMETHROWER_ALTFIRE2, 0 ); break;
		case 2: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAMETHROWER_ALTFIRE3, 0 ); break;
		case 3: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAMETHROWER_FIRE1, 0 ); break;
		case 4: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAMETHROWER_FIRE2, 0 ); break;
		case 5: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAMETHROWER_FIRE3, 0 ); break;
		case 6: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAMETHROWER_FIRE4, 0 ); break;
		}
		
		V_PunchAxis( 0.0, -10.0 );
	}

	gEngfuncs.pEventAPI->EV_PlaySound( iIndex, vecOrigin, CHAN_WEAPON, "weapons/fla_fire.wav", 1, ATTN_NORM, 0, 95 + gEngfuncs.pfnRandomLong( 0, 10 ) );
}
...
void EV_FireSTI( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? STI_ATTACK_SLIDEBACK : STI_ATTACK, 2 );
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/sti_fire.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
...
void EV_FireMP5Navy( event_args_t *args )
{
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m161.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m162.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
