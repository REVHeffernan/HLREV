...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	else if ( iTextToShow == SHOW_PAPER )
	{
		if ( !m_szPaperTitle || !m_szPaperTitle[0] )
			strcpy( cTitle, "Unknown title" );
		else
			strncpy( cTitle, m_szPaperTitle, MAX_TITLE_LENGTH );
...
void TeamFortressViewport::ShowVGUIMenu( int iMenu )
{
	...
	case MENU_PAPER:
		pNewMenu = CreateTextWindow( SHOW_PAPER );
		break;
...
void TeamFortressViewport::paintBackground()
{
	...
	if ( m_pEntityInfo )
	{
		m_pEntityInfo->Update();
		m_pEntityInfo->setVisible( true );
	}
...
int TeamFortressViewport::MsgFunc_PaperTitle( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	strncpy( m_szPaperTitle, READ_STRING(), MAX_PAPER_TITLE_LENGTH );

	return 1;
}

void TeamFortressViewport::CreateEntityInfo()
{
	m_pEntityInfo = new CVGUIEntityInfo();
	m_pEntityInfo->setParent( this );
}

void TeamFortressViewport::CreateTimer()
{
	m_pTimer = new CVGUITimer();
	m_pTimer->setParent( this );
}

int TeamFortressViewport::MsgFunc_EntInfo( const char *pszName, int iSize, void *pbuf )
{
	if ( m_pEntityInfo )
		return m_pEntityInfo->MsgFunc_EntInfo( pszName, iSize, pbuf );

	return 0;
}

int TeamFortressViewport::MsgFunc_Paper( const char *pszName, int iSize, void *pbuf )
{
	if ( m_iGotAllPaper )
		m_szPaper[0] = 0;
	...
	m_iGotAllPaper = READ_BYTE();

	int iRoomInArray = sizeof( m_szPaper ) - strlen( m_szPaper ) - 1;

	strncat( m_szPaper, READ_STRING(), iRoomInArray >= 0 ? iRoomInArray : 0 );
	m_szPaper[sizeof( m_szPaper ) - 1] = '\0';

	// Don't show papers for HLTV spectators
	if ( m_iGotAllPaper && !gEngfuncs.IsSpectateOnly() )
		ShowVGUIMenu( MENU_PAPER );

	return 1;
}

int TeamFortressViewport::MsgFunc_Timer( const char *pszName, int iSize, void *pbuf )
{
	if ( m_pTimer )
		return m_pTimer->MsgFunc_Timer( pszName, iSize, pbuf );
